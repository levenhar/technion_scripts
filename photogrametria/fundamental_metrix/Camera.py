...
class Camera(object):
    ...
    def ChessCalibration(self, imagesList):
        pattern_size = (9, 7)  # Number of inner corners in each row and column
        square_size = 1.0  # Size of each square in the chessboard pattern (in arbitrary units)
        ...
        image_size = gray.shape[::-1]  # Assuming all images have the same size
        _, k, _, r_vecs, t_vecs = cv2.calibrateCamera(object_points, image_points, image_size, None, None)

        self.focalLength = k[0,0]
        self.__principalPoint = k[:2,2]
