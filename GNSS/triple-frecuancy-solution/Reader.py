...
def readsp3(SP3NameFile):
    ...
    for i in range(len(ar)):
        if ar[i,0][0]=="*":
            c=-1
            q=0
            CoordinatesAll = np.zeros((32,4,95))
            for j in range(i,len(ar),33):
                ...
                if len(Coordinates) == 31:
                    Coordinates = np.concatenate((Coordinates, np.array([[0,0,0,0, 0]])), axis=0)
                elif len(Coordinates) != 32:
                    x=0
                try:
...
def readOfile(OfailName):
    ...
    for i in range(len(ar)):
        if ar[i] == "END OF HEADER":
            ...
            j=i+1
            #len(ar)
            while j<len(ar)-1:# in range(i+1,len(ar)):
                if j>len(ar)-1:
                    break
                #header of epoch
                Inf = ar[j, 0][:31].split(" ")
            ...
            return Observations.astype(np.float) , np.array(satelliteNum), info , aprx.astype(np.float)
